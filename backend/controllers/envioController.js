...
const calcularEnvio = async (req, res) => {
  const { departamento, subtotal } = req.query;
  try {
    const costo = calcularCostoEnvio(departamento || "", Number(subtotal) || 0);
    res.json({ ok: true, departamento: departamento || "", costo, gratis: costo === 0 });
  } catch (err) {
